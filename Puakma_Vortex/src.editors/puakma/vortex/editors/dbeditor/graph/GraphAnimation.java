package puakma.vortex.editors.dbeditor.graph;
...
/**
 * TODO: in 3.3 revise this exmaple class from flow example
 * 
...
public class GraphAnimation
{
  ...
  public static boolean playbackState(IFigure container)
  {
    ...
    for(int i = 0; i < children.size(); i++) {
      ...
      if(rect2 == null)
        continue;
      child.setBounds(new Rectangle((int) Math.round(progress * rect2.x + (1 - progress)
                                                     * rect1.x), (int) Math
          .round(progress * rect2.y + (1 - progress) * rect1.y), (int) Math
          .round(progress * rect2.width + (1 - progress) * rect1.width), (int) Math
          .round(progress * rect2.height + (1 - progress) * rect1.height)));
      // child.invalidate();
    }
    return true;
  }
